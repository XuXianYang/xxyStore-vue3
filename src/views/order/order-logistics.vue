<template>
  <xtx-dialog title="查看物流" v-model:visible="dialogVisible">
...
export default {
  setup() {
    ...
    const open = order => {
        ...
        getLogistics(order.id)
    }
    
    const getLogistics = orderId => {
        logisticsOrder(orderId).then(res=>{
            logistics.value = res.result
        })
    }
    return { dialogVisible, logistics,open };
  },
};
</script>
